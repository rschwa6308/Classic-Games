...
def display(board):
    ...
    #draw 'king squares' on kings
    for king in kings:
        pygame.draw.rect(screen,yellow,Rect(float(king[0])/8.0*s_width+s_width/24,float(king[1])/8.0*s_height+s_height/24,s_width/24,s_height/24))
    #update display
...
def is_move(piece,tar,turn,kings):
    #print piece
    #print tar
    px = piece[0]
    py = piece[1]
    ...
    #if kings
    if (piece[0],piece[1]) in kings:
        #print "king detected"
        #direct move
        if abs(ty-py) == 1 and abs(px-tx) == 1:
            return True
        #single jump
        if abs(ty-py) == 2 and tx-px != 0:
            #left
            if tx-px == -2:
                if 0!= board[py-1][px-1] != turn:           #up
                    board[py-1][px-1] = 0
                    try: kings.remove((px-1,py-1))
                    except: pass
                    return True
                if 0!= board[py+1][px-1] != turn:           #down
                    board[py+1][px-1] = 0
                    try: kings.remove((px-1,py+1))
                    except: pass
                    return True
